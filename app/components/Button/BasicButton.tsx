...
const BasicButton = (props: any) => {
    const { onClick, rounded, disabled, variant, startIcon, endIcon,hover,tooltip,size, ...restProps } = props
    ...
    return (
        <ThemeProvider theme={muiTheme}>
            <Tooltip title={tooltip}>
                <Button size={size} sx={{ opacity: 1,zIndex: 999}} startIcon={startIcon} endIcon={endIcon} disabled={disabled} variant={variant || 'contained'} color="primary" className={`${rounded ? " rounded-full" : ""}`} onClick={(e) => { onClickEvent(e) }}>{props.children}</Button>
